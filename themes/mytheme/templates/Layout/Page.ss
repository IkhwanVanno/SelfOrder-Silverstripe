<div class="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-4 md:gap-6">
    <% loop Produk %>
    <div class="bg-light rounded-2xl shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1 overflow-hidden group">
        <!-- Product Image -->
        <div class="relative overflow-hidden">
            ...
            <div class="absolute top-2 left-2">
                <span class="bg-primary text-white text-xs font-bold px-2 py-1 rounded-full">Tersedia</span>
            </div>
        </div>
        ...
        <div class="p-3 md:p-4">
            ...
            <!-- Quantity Controls -->
            <div class="flex items-center justify-center bg-white rounded-full border-2 border-accent p-2 shadow-sm">
                <% if $Top.getCartQuantity($ID) %>
                    ...
                    <form method="post" action="$BaseHref/keranjang/update-cart" class="inline">
                        ...
                        <button type="submit" 
                                class="w-8 h-8 bg-accent text-primary rounded-full flex items-center justify-center hover:bg-accent/80 transition-all duration-200 hover:scale-110">
                    ...
                    <form method="post" action="$BaseHref/keranjang/update-cart" class="inline">
                        ...
                        <button type="submit" 
                                class="w-8 h-8 bg-secondary text-white rounded-full flex items-center justify-center hover:bg-secondary/80 transition-all duration-200 hover:scale-110">
                            <i class="fas fa-plus text-xs"></i>
                        </button>
                    </form>
                <% else %>
                    <!-- Add to Cart -->
                    <button class="w-8 h-8 bg-gray-200 text-gray-400 rounded-full flex items-center justify-center cursor-not-allowed" 
                            disabled>
                        <i class="fas fa-minus text-xs"></i>
                    </button>
                    ...
                    <form method="post" action="$BaseHref/keranjang/add-to-cart" class="inline">
                        ...
                        <button type="submit" 
                                class="w-8 h-8 bg-secondary text-white rounded-full flex items-center justify-center hover:bg-secondary/80 transition-all duration-200 hover:scale-110 hover:shadow-md">
                            <i class="fas fa-plus text-xs"></i>
                        </button>
                    </form>
                <% end_if %>
